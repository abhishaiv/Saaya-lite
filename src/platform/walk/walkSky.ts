 ...
 *
 * The camera has turned to her heading since 2026-09-23, so it is no longer rigid to her, but
 * nothing below moves with that turn: the boom keeps its length, its pitch and its aim point,
 * and yaw moves none of the three. The horizon therefore still sits at a fixed row and the sky
 * is still the strip above it. That strip's top is `fov / 2` above the camera's own axis, and
 * the axis is depressed by the angle its aim point sits below it - both read from the camera's
 * facts, so a camera amendment moves the sky with it.
 */
