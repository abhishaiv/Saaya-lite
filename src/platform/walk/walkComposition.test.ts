 ...
 * her feet 0.736, her head 0.616, her height 0.120. Her feet are the stable measurement, within
...
const REFERENCE_HEAD = 0.616; // GROUNDED-EXEMPT: measured off the reference frames; see the block above.
const REFERENCE_CHARACTER = 0.120; // GROUNDED-EXEMPT: measured off the reference frames; see the block above.
 ...
 *
 * The camera is a rigid offset from her: `dist` back along an axis `pitch` above the horizon,
 * aimed at a point `look_at` above her feet. Everything here follows from those four numbers
 * and her height, in the same order `walkScene.ts` applies them: the camera sits `dist * sin
 * pitch` up and `dist * cos pitch` back and looks down its own axis, and a row is where a ray
 * that many degrees off that axis lands once the renderer projects it.
 */
function frame(): {
  horizon: number;
  feet: number;
...
describe("the frame's own ground footprint", () => {
  it("shows about six metres of ground across at her feet", () => {
    expect(groundWidthAtFeetRow()).toBeCloseTo(5.95, 2); // GROUNDED-EXEMPT: solved by the function above from the camera facts, not a product value.
  });
