...
| Class | Width dp | Example | Priority |
|---|---|---|---|
...
**Design at 360 dp.** Verify at 320 dp before calling any screen done.

## Rules by breakpoint

| Element | 320 - 359 | 360 - 479 | 480+ |
|---|---|---|---|
| Screen padding | 16 dp | 20 dp | 24 dp |
| `LadderCard` horizontal margin | **20 dp** | 30 dp | max width 420 dp, centred |
| Card padding | 18 dp | 22 dp | 22 dp |
| `CountdownRing` in card | 72 dp | 88 dp | 88 dp |
| `BigActionButton` height | 64 dp | 72 dp | 72 dp |
| `display` type | 30 sp | 34 sp | 34 sp |
| Bottom sheet peek | 148 dp | 160 dp | 160 dp |
| `StatRow` per row | 2 | 3 | 3 |

Below 360 dp, if the check-in card would exceed 70% of screen height, drop the 40 dp hero
...
portrait layout centred at a 480 dp max content width.
...
| Rule | Detail |
|---|---|
| Use `sp` for all text | never `dp` for type |
| Use `dp` for icons and spacing | so layout does not explode with type |
...
| `display` numerals | cap at 1.5x, since a countdown at 2.0x on a 320 dp screen has nowhere to go |

At 2.0x on a 320 dp screen, the check-in card is allowed to scroll internally. The primary
...
- Status bar: `StatusPill` sits 12 dp below the top inset.
- Navigation bar: bottom sheet and every primary button sit above the bottom inset.
- Gesture navigation: keep 16 dp clear of the bottom edge so the sheet drag does not fight
...
because it lives in the foreground service and not in the activity. That is already true if
...
| APK size | under 25 MB. Reasonable, not tuned. |
