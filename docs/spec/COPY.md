...
| `onb_contact_body` | One person is enough. If you miss two check-ins, Saaya shows the message they would receive. | ఒక్కరు చాలు. మీరు రెండు చెక్-ఇన్‌లు మిస్ అయితే, వారు పొందే సందేశాన్ని సాయ చూపిస్తుంది. |
...
| `onb_tour_body` | Before you need it, run the guided demo on the map. | అవసరం రాకముందే మ్యాప్‌లో గైడెడ్ డెమో నడపండి. |
...
| `status_family` | Preparing a message | సందేశం సిద్ధం చేస్తోంది |
...
| `warn_queue_failed` | **Cut in Lite.** No alert is sent or queued in this round. | **Lite లో లేదు.** ఈ రౌండ్‌లో హెచ్చరిక పంపబడదు లేదా క్యూలో ఉంచబడదు. |
| **Zone detail** | | |
...
| `sos_local_only` | SOS is active. This beta does not send a report. Choose a call below to open your phone's dialler. | SOS యాక్టివ్‌గా ఉంది. ఈ బీటా నివేదికను పంపదు. మీ ఫోన్ డయలర్‌ను తెరవడానికి దిగువన ఉన్న కాల్ ఎంపికను ఉపయోగించండి. |
| `sos_local_offline` | No network is needed for this local-only SOS. Use a call option below. | ఈ స్థానిక SOS కు నెట్‌వర్క్ అవసరం లేదు. దిగువన ఉన్న కాల్ ఎంపికను ఉపయోగించండి. |
...
| **State-view rows** | **Cut, round two. These keys do not ship in Lite.** | **రాష్ట్ర వ్యూ వరుసలు Lite లో లేవు.** |
...
## Notification kinds — cut from Lite

Lite uses in-page check-ins only and does not request notification permission. The former
Notification API rows below are retained as a round-two reference, not current behaviour.
...
Write fresh copy **only** for moments Lite has that iOS does not: the demo panel, the zone
map's hour context and the prototype disclosures. The state view is cut from Lite.
...
| `cd_police_view` | **Cut in Lite.** No state view is available. | **Lite లో లేదు.** రాష్ట్ర వ్యూ అందుబాటులో లేదు. |
...
| `cd_cancel_escalation` | Cancel this preview. It stays on your phone. | ఈ ప్రివ్యూను రద్దు చేయి. ఇది మీ ఫోన్‌లోనే ఉంటుంది. |
