# Saaya Lite - Demo Video Script
Three minutes maximum, per the brief. Shot list, timings and exact narration.
Recorded on E9. Do not improvise on the day.
...
| State view | **Cut in Lite.** Do not open, film or imply a console. |
...
| 1:38 - 1:58 | Family escalation preview, red, the exact message, the disclosure banner and both device-handoff controls in frame | "Saaya prepares the area, the hour, that area's record, and the two missed checks. Not a blank alarm. Saaya does not send it. Until separate Chrome phone observation confirms each installed composer, describe each control only as an attempted navigation with the exact message ready." |
| 1:58 - 2:12 | Cancel window lapses. SOS, instant, red, with the local-only disclosure and dial actions in frame. | "She still does not answer. Now, and only now, SOS. The prototype says plainly that it does not send a report, then puts 112, 181 and the nearest-station dial handoff in her hands." |
| 2:12 - 2:30 | Hold on the SOS screen, then the About screen's limits | "Saaya Lite is a prototype. The message handoff is user-controlled: her tap attempts to open her own app, but Saaya never sends it or knows whether it arrived. Every limit is written into the product, not just the write-up." |
...
- **Show the demo banner.** Never crop or hide it. A judge who spots a concealed demo mode
  discounts everything else.
- **Never speed up the video to fake responsiveness.** Demo speed is a labelled product
  setting, and that is honest. Editing the footage to look faster is not.
- Say the government numbers **as the governments published them**, and cite on screen.
- No music over the SOS section. Silence is stronger and it is respectful.
- Show the real device, real fingers, one hand. This is a one-handed product.
- End on the disclaimer, not on the logo. Honesty is a scored criterion.
